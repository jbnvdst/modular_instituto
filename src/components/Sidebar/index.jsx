...
const Sidebar = ({ isOpen, setIsOpen }) => { 
  ...
  // Rol normalizado y flag de admin
        ...
        <nav className="w-full flex flex-col items-center gap-16">
          ...
          <ul className="flex flex-col w-full">
            <li className="w-full">
              ...
              >
                <GoHome className={({ isActive }) => ` ${isActive ? activeStyle : unactiveStyle}`} size={22}/>
                <span className="font-medium">Inicio</span>
              </NavLink>
            </li>

            {/* Semáforos solo para admin */}
            {isAdmin && (
              <li className="w-full">
                <NavLink
                  to="/semaphores"
                  className={({ isActive }) => `flex gap-2 w-full py-2 px-6 ${isActive ? activeStyle : unactiveStyle}`}
                >
                  <BsStoplights className={({ isActive }) => ` ${isActive ? activeStyle : unactiveStyle}`} size={22} />
                  <span className="font-medium">Semáforos</span>
                </NavLink>
              </li>
            )}
            ...
            <li className="w-full">
              <NavLink to="/tasktemplates" className={({ isActive }) => `flex gap-2 w-full py-2 px-6 ${isActive ? activeStyle : unactiveStyle}`}>
                <CgTemplate className={({ isActive }) => ` ${isActive ? activeStyle : unactiveStyle}`} size={22} />
                <span className="font-medium">Plantillas de Tareas</span>
              </NavLink>
            </li>
            <li className="w-full">
              <NavLink to="/recurringtasks" className={({ isActive }) => `flex gap-2 w-full py-2 px-6 ${isActive ? activeStyle : unactiveStyle}`}>
                <LuCalendarClock className={({ isActive }) => ` ${isActive ? activeStyle : unactiveStyle}`} size={22} />
            ...
            {isAdmin && (
              <li className="w-full">
                <NavLink to="/admin" className={({ isActive }) => `flex gap-2 w-full py-2 px-6 ${isActive ? activeStyle : unactiveStyle}`}>
                  <LuUserRoundCog className={({ isActive }) => ` ${isActive ? activeStyle : unactiveStyle}`} size={22} />
